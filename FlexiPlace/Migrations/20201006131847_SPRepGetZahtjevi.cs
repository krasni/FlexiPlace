...
{
    public partial class SPRepGetZahtjevi : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            var sp = @"ALTER PROCEDURE [dbo].[spRepGetZahtjevi]
	@Status nvarchar(max),
	@Ispravnost nvarchar(max),
...
BEGIN
	...
	FROM
		...
		LEFT JOIN Ispravnost i ON z.IspravnostId = i.Id
	WHERE
		s.Naziv IN ( SELECT * FROM dbo.Test_Split(@Status))
		AND
			i.IspravnostStatus IN ( SELECT * FROM dbo.Test_Split(@Ispravnost))
		AND 
