...
{
    public partial class DodaoUlogaHanfaStora : Migration
    {
...
FROM	
	BP22T.dbo.vViewDjelatnici d 
  	LEFT JOIN [BP22T].[HumanResources].[EmployeeOrganizationUnit] eou ON d.IDZaposlenog = eou.EmployeeId
...
WHERE
	...
	d.Username = @EmployeeADName
	AND
	(eou.EndDate>getdate() or eou.EndDate is null) and (eou.StartDate<GETDATE())
	AND
    uz.Active = 1
